...
public class PercolationStatsQF {
    ...
    public PercolationStatsQF(int N, int T) {
        ...
        for (int i=0; i < T; i++){
            PercolationQF percolationQF = new PercolationQF(N);
            while (!percolationQF.percolates()) {
                percolationQF.open(StdRandom.uniform(N), StdRandom.uniform(N));
            }
            results[i] = (double)percolationQF.numberOfOpenSites() / (N*N);
        }
